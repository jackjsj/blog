...
1. 二进制传输：是HTTP2.0中所有加强性能的核心，在之前的HTTP版本是通过文本的方式传输数据，而2.0引入了新的编码机制，所有传输数据都会被分割，并采用二进制格式编码
2. 多路复用：就是在一个TCP连接中可以存在多个请求，对端可以通过帧中的标识知道属于哪个请求，通过这个技术，避免了队头阻塞的问题，极大提高传输性能。
3. 头部压缩：2.0使用了HPACK压缩格式对传输的header进行编码，减少了header的大小，并在两端维护了索引表，用于记录出现过的header，后面如果header的键值没变，就可以只传输键名。
