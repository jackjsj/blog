...
2. `http-equiv`, 设置**http的响应头报文**，如设置**http缓存过期时间**
3. `viewport`，常用于**移动开发**，可以**控制视口的大小和比例**，**用户是否可缩放视口**等。
...
2. localStorage和sessionStorage差不多，一个是**永久存储**，除非手动删除，一个是**会话级存储**，即页面关闭数据就没了。
3. indexedDB是H5的**数据库储存**方案，允许存大容量数据，用**key-value**进行存储，用**JS操作方便**
...
- 伪元素是**不存在于**文档树中的元素，通过给它**添加样式来实现一些效果**，如before,after
...
当调用栈中的代码**全部执行完毕后**，js引擎才会**去任务队列中找**有没有异步任务，如果有就**依次将它们放入调用栈中**执行，这个过程是循环不断的。****
...
- 概述
  ...
  - ViewModel是**View**和**Model**层的**桥梁**，**数据绑定在ViewModel层**会**自动**将数据渲染到页面，**双向绑定**的**视图变化**的时候也会**通知****ViewModel**层**更新数据**。
- 优点
...
- created：发生在**实例创建后**，这时候已**完成数据监测**，可**使用和获取**数据，但**无法与DOM交互**。
...
- 具体实现
  ...
  - 最后实现一个`MVVM`入口，用来**接收数据对象**，**指定视图模板**，调度`Observer`，`Compile`和`Watcher`。
...
- 回流：是指**部分渲染树**或者**整个渲染树**需要**重新分析，重新计算节点尺寸**，表现为**重新布局和重新排列元素**
...
- GET/POST区别：
  ...
  - **数据类型**不同：GET只支持ASCII码的数据类型，而POST无限制
...
2. 多路复用：一个TCP连接中可以**存在多个请求**，接收方可以通过**帧中的标识**知道属于哪个请求，解决了**队头阻塞**的问题，提高传输效率。
...
- 根据**配置文件**和命令中**获取合并**参数，得到**最终参数**
- 初始化**Compile对象**，加载所有**插件**，执行**run方法**开始**编译**
- 从配置中获取entry**入口文件**
- **从入口文件出发**，**递归**查出**依赖的模块**，构建**依赖关系图**
- 使用**Loader**对这些**模块**进行**翻译**
- 得到**翻译后的结果**和**依赖关系**
- **根据入口文件和模块间的依赖关系**，**生成**一个个**包含多个模块的chunk**，将chunk**转换**成**独立的文件**，放入**输出列表中**
